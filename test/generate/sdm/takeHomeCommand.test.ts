...
import { takeHomeCommand } from "../../../lib/generate/sdm/takeHomeCommand";

describe("take home transforms", () => {

    // tslint:disable-next-line:deprecation
    const transform = chainTransforms(...(takeHomeCommand.transform as Array<CodeTransform<NodeProjectCreationParameters>>));

    const parameters: NodeProjectCreationParameters & SeedDrivenGeneratorParameters = {
        appName: "appName",
        version: "1.0.1",
        screenName: "rod",
        source: {
    ...
    it("should change name", async () => {
        const p = InMemoryProject.of({ path: "package.json", content: PackageJson });
        ...
        assert.strictEqual(json.name, "appName");
    ...
    it("should remove dependency from package-lock.json", async () => {
        const p = InMemoryProject.of({ path: "package-lock.json", content: PackageJson }, {
    ...
    it("should remove import from machine.ts", async () => {
        ...
        assert(!newMachine.includes("import { ConfigurationPack }"), newMachine);
    ...
    it("should remove pack usage from machine.ts", async () => {
        ...
        await transform(p, { parameters } as any, parameters);
        const newMachine = p.findFileSync("lib/machine.ts").getContentSync();
        assert(!newMachine.includes("sdm.addExtensionPacks(ConfigurationPack)"), newMachine);
    });
...
const PackageJson = `{
  ...
  "dependencies": {
    ...
    "@atomist/sdm-pack-configuration": "1.3.0-master.20190203162110",
